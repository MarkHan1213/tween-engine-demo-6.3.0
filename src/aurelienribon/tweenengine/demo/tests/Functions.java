...
public class Functions extends Test {
	...
	protected void initializeOverride() {
		...
		functions1 = atlas.createSprite("functions1");	// 指的是四个词 quad cubic quart quint
		functions2 = atlas.createSprite("functions2");	// 指的是三个词 circ sine expo linear
		functions3 = atlas.createSprite("functions3");	// 指的是三个词 back elastic bounce
	...
	private void startFunctions1(float delay) {
		...
		Timeline.createParallel()
			...
			.push(Tween.to(sprites[3], SpriteAccessor.OPACITY, 0.4f).target(1))	// 显示第四个sprite.
			.start(tweenManager);
	...
	private void startFunctions3(float delay) {
		...
		Timeline.createParallel()
			...
			.push(Tween.to(sprites[3], SpriteAccessor.OPACITY, 0.4f).target(0))	// 隐藏第四个sprite.这里不需要第四个sprite
			.start(tweenManager);
